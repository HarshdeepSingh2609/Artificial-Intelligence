...
moves.push_back(i);}} 
return moves;} 
int minimax(vector<int>& board, int depth, int maxDepth) { minimaxCalls++;  
int userWin = checkWin(board, User); 
int computerWin = checkWin(board, Computer); 
if (userWin == 10) return -10;  
if (computerWin == 10) return 10;  
if (isDraw()) return 0;  
if (depth >= maxDepth) return rate(board, Computer, turn);  int bestScore = INT_MIN;  
...
for (int move : moves_left(board)) { 
board[move] = Computer;  
int score = minimax(board, 0, maxDepth) + rate(board, Computer, turn);  board[move] = 2;  
if (score > bestScore) { 
bestScore = score; 
toMove = move; }} 
if (toMove != -1) { 
...
cout << "Total minimax calls: " << minimaxCalls << endl;  
return 0; 
} 
